...
(OpenAI / OpenRouter / Gemini) e integra com o GitHub CLI (`gh`) para push + PR. Binário único, sem servidor/DB.
...
- Lint: `go vet ./...`
- Test: `go test ./...`
- Instalar o binário: `go install ./cmd/ob` (instala como `openbench` em `$(go env GOPATH)/bin`)
- Rodar: `ob --help` ou `openbench --help` (alias `ob` opcional via `install.sh`)
